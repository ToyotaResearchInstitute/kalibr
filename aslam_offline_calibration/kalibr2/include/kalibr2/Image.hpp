...
To C++. With some simplifications:
  1. No support for parallel processing of the images (dropped because for the moment we can only read images sequentially)
  2. We don't use the findTarget with transformation, since it's not being used in kalibr_calibrate_cameras code which is the part we currently care about.
  3. We always clear the image in the observation to save space, no reason to keep it around.
*/
