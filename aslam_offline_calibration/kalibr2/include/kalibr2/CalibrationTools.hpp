...
template <typename CameraGeometryT, typename DesignVariableT,
          typename ReprojectionErrorT>
bool CalibrateIntrinsics(
  ...
  std::vector<boost::shared_ptr<aslam::backend::TransformationBasic>>
      target_pose_dvs;
  std::vector<boost::shared_ptr<ReprojectionErrorT>> reprojection_errors;
  for (const auto& obs : observations) {
    ...
    for (size_t i = 0; i < target->size(); ++i) {
      ...
      if (valid) {
        auto rerr = boost::make_shared<ReprojectionErrorT>(
...
bool CalibrateIntrinsics(
    ...
    const boost::shared_ptr<CameraGeometryT>& geometry,
    const aslam::cameras::GridDetector& detector) {
  return CalibrateIntrinsics<CameraGeometryT, DesignVariableT,
                             ReprojectionErrorT>(observations, geometry,
